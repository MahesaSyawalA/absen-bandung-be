...
router.post('/', async (req, res) => {
    try {
        const { date, status, user_id, photo_path, latitude, longtitude } = req.body;
        
        // Validasi input
        if (!date || !status || !user_id || !photo_path || !latitude || !longtitude ) {
            return response(400, "Semua field harus diisi", "ERROR", res);
        }

        const newAttendance = await Attendance.add(date, status, user_id, photo_path, latitude, longtitude);
